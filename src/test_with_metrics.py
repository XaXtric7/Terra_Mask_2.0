...
def calculate_dice_coefficient(y_true, y_pred, class_id):
    ...
    dice = 2.0 * intersection / \
        (true_mask.sum() + pred_mask.sum()
         ) if (true_mask.sum() + pred_mask.sum()) > 0 else 1.0
    return dice
...
if __name__ == "__main__":
    ...
    # size of each patch and window
    patch_size = slice_config['vars']['patch_size']
    # the backbone/encoder of the model
    ...
    logger = custom_logger(
        "Land Cover Semantic Segmentation Test Logs", log_path, log_level)

    # get the dir of input images for inference from config
    img_dir = ROOT / slice_config['dirs']['data_dir'] / \
        slice_config['dirs']['test_dir'] / slice_config['dirs']['image_dir']
    img_dir = img_dir.as_posix()

    # get the dir of input masks for inference from config
    gt_mask_dir = ROOT / slice_config['dirs']['data_dir'] / \
        slice_config['dirs']['test_dir'] / slice_config['dirs']['mask_dir']
    ...
    pred_mask_dir = ROOT / \
        slice_config['dirs']['output_dir'] / \
        slice_config['dirs']['pred_mask_dir']
    # make the directory if it does not exist
    ...
    pred_plot_dir = ROOT / \
        slice_config['dirs']['output_dir'] / \
        slice_config['dirs']['pred_plot_dir']
    # make the directory if it does not exist
    ...
    # Chage made to make it work
    # model = torch.load(model_path, map_location=torch.device(device))
    model = torch.load(model_path, map_location=torch.device(
        device), weights_only=False)

    class_values = [Constants.CLASSES.value.index(
        cls.lower()) for cls in classes]
    num_classes = len(class_values)

    img_list = list(filter(lambda x: x.endswith(
        (file_type)), os.listdir(img_dir)))
    ...
    try:
        for filename in img_list:
            ...
            logger.info(
                f"Preparing image and ground truth mask file {filename}...")
            ...
            try:
                ...
                logger.error(
                    f"Could not read ground truth mask file {filename}!")
                raise e

            # padding image to be perfectly divisible by patch_size
            try:
                pad_height = (
                    math.ceil(image.shape[0] / patch_size) * patch_size) - image.shape[0]
                pad_width = (
                    math.ceil(image.shape[1] / patch_size) * patch_size) - image.shape[1]
            ...
            try:
                patches = patchify(
                    image_padded, (patch_size, patch_size, 3), step=patch_size//2)[:, :, 0, :, :, :]
                mask_patches = np.empty(
                    patches.shape[:-1], dtype=patches.dtype)
            except Exception as e:
            ...
            try:
                for i in tqdm(range(0, patches.shape[0])):
                    for j in range(0, patches.shape[1]):
                        ...
                        img_patch = img_patch.transpose(
                            2, 0, 1).astype('float32')
                        x_tensor = torch.from_numpy(
                            img_patch).to(device).unsqueeze(0)
            ...
            try:
                ...
                print(
                    f"Total classes found in the predicted mask: {classes_found}")
                logger.info(
                    f"Total classes found in the predicted mask: {classes_found}")
            except Exception as e:
            ...
            try:
                ...
                pred_mask = pred_mask.argmax(2)
                print(
                    f"Classes present in the predicted mask after filtering according to user input of 'test_classes': {classes}")
                logger.info(
                    f"Classes present in the predicted mask after filtering according to user input of 'test_classes': {classes}")
            except Exception as e:
                logger.error(
                    "Could not filter user given classes from the predicted mask!")
                raise e
            ...
            try:
                ...
                plot_fig.savefig(os.path.join(
                    pred_plot_dir, filename.split('.')[0] + '.png'))
                print("Prediction plot saved successfully!")
                logger.info("Prediction plot saved successfully!")
            except Exception as e:
                logger.error(
                    "Could not plot the image, ground truth mask, and predicted mask!")
                raise e
            ...
            try:
                ...
                for i, class_name in enumerate(classes):
                    ...
                    ap_50 = calculate_average_precision(
                        gt_mask, pred_mask, i, threshold=0.5)
                    map_50_scores.append(ap_50)

                    # mAP 75 (threshold 0.75)
                    ap_75 = calculate_average_precision(
                        gt_mask, pred_mask, i, threshold=0.75)
                ...
                print(
                    f"  Pixel Accuracy: {image_metrics['pixel_accuracy']:.4f}")
        ...
        try:
            ...
            for i, class_name in enumerate(classes):
                ap_50 = calculate_average_precision(
                    all_gt_masks, all_pred_masks, i, threshold=0.5)
                map_50_scores.append(ap_50)

                ap_75 = calculate_average_precision(
                    all_gt_masks, all_pred_masks, i, threshold=0.75)
                map_75_scores.append(ap_75)

                dice = calculate_dice_coefficient(
                    all_gt_masks, all_pred_masks, i)
                dice_scores.append(dice)
            ...
            print(
                f"  Mean Pixel Accuracy: {overall_metrics['mean_pixel_accuracy']:.4f}")
            ...
            print(
                f"  Frequency Weighted IoU: {overall_metrics['frequency_weighted_iou']:.4f}")

            logger.info("Overall Metrics Across All Test Images:")
            logger.info(
                f"  Pixel Accuracy: {overall_metrics['pixel_accuracy']:.4f}")
            logger.info(
                f"  Mean Pixel Accuracy: {overall_metrics['mean_pixel_accuracy']:.4f}")
            ...
            for class_name in classes:
                print(f"  {class_name}:")
                print(
                    f"    IoU: {overall_metrics['per_class_iou'][class_name]:.4f}")
                print(
                    f"    AP@50: {overall_metrics['per_class_map_50'][class_name]:.4f}")
                print(
                    f"    AP@75: {overall_metrics['per_class_map_75'][class_name]:.4f}")
                print(
                    f"    Dice: {overall_metrics['per_class_dice'][class_name]:.4f}")
            ...
            per_class_csv = os.path.join(
                metrics_dir, 'test_metrics_per_class.csv')

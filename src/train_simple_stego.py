"""
Training script for simplified STEGO model
"""
...
from models.simple_stego import SimpleSTEGOModel, SimpleSTEGOTrainer
...
class SimpleSTEGODataset:
    """
    Custom dataset for simplified STEGO training
    """
...
def train_simple_stego_model():
    """
    Main training function for simplified STEGO model
    """
    # Load configuration
    ...
    log_path = log_dir / "simple_stego_train.log"
    logger = custom_logger("Simple STEGO Training Logs", log_path.as_posix(), log_level)
    ...
    # Create dataset
    train_dataset = SimpleSTEGODataset(
        str(patches_img_dir),
    ...
    # Create simplified STEGO model
    logger.info("Creating simplified STEGO model...")
    ...
    # Create trainer
    trainer = SimpleSTEGOTrainer(model, learning_rate=0.001, device=device)
    
    # Training loop
    logger.info("Starting simplified STEGO training...")
    train_losses = []
    train_accuracies = []
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % 5 == 0:
            ...
            torch.save(checkpoint, model_dir / f"simple_stego_model_epoch_{epoch+1}.pth")
    ...
    model_dir = ROOT / slice_config['dirs']['model_dir']
    torch.save(final_checkpoint, model_dir / "simple_stego_model_final.pth")
    ...
    plt.tight_layout()
    plt.savefig(model_dir / "simple_stego_training_curves.png")
    plt.close()
    
    logger.info("Simplified STEGO training completed successfully!")


if __name__ == "__main__":
    train_simple_stego_model()
